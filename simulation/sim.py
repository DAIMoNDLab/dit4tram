...
class Simulation:
    ...
    def get_last_step_mean_speed(self, detector_ids = None, flatten = False) -> dict:
        detector_ids = list(self.available_detectors.keys()) if detector_ids == None else detector_ids
        detector_ids = [detector_ids] if not isinstance(detector_ids, list) else detector_ids

        mean_speeds = [] if flatten else {}
        for detector_id in detector_ids:
            ...
            if detector_type == "inductionloop":
                detected_speed = traci.inductionloop.getLastStepMeanSpeed(detector_id)
            elif detector_type == "multientryexit":
                detected_speed = traci.multientryexit.getLastStepMeanSpeed(detector_id)
            else:
                warn("Warning: Unknown detector type '"+detector_type+"'")

            if flatten: mean_speeds += detected_speed
            else: mean_speeds[detector_id] = detected_speed

        if flatten: mean_speeds = list(set(mean_speeds)) # Assert only unique IDs. Why IDs found at multiple detectors?

        return mean_speeds
    
    def get_last_step_vehicle_number(self, detector_ids = None, flatten = False) -> dict:
        detector_ids = list(self.available_detectors.keys()) if detector_ids == None else detector_ids
        detector_ids = [detector_ids] if not isinstance(detector_ids, list) else detector_ids

        vehicle_counts = [] if flatten else {}
        for detector_id in detector_ids:
            ...
            if detector_type == "inductionloop":
                detected_number = traci.inductionloop.getLastStepVehicleNumber(detector_id)
            elif detector_type == "multientryexit":
                detected_number = traci.multientryexit.getLastStepVehicleNumber(detector_id)
            else:
                warn("Warning: Unknown detector type '"+detector_type+"'")

            if flatten: vehicle_counts += detected_number
            else: vehicle_counts[detector_id] = detected_number

        if flatten: vehicle_counts = list(set(vehicle_counts)) # Assert only unique IDs. Why IDs found at multiple detectors?
    ...
    def take_screenshot(self):
        if self.running:
            traci.gui.screenshot("View #0", "./screenshots/screenshot_{0}.png".format(self.curr_step))
...
def set_state(sim, sc_state, M_map):
    """
    Permanently sets simulation phases to current controller state
    :param sim: Current simulation object
    :param sc_state: Signal controller state array
    :param M_map: Movement mapping dictionary
    """
    
    phase_str_len = set([len(m) for m in M_map.values()])
    if len(phase_str_len) != 1: raise ValueError
    else: phase_str_len = list(phase_str_len)[0]
    phase_arr = [0]*phase_str_len

    for idx, m in enumerate(sc_state):
        m_mask = M_map[idx]

        for i in range(phase_str_len):
            if m_mask[i] == "1":
                if 'g' in m: light_setting = 'G'
                elif 'y' in m: light_setting = 'y'
                elif 'r' in m: light_setting = 'r'
                else: raise ValueError
                phase_arr[i] = light_setting
            if m_mask[i] == "2": #right on red, for bicycles
                if 'g' in m: light_setting = 'G'
                elif 'y' in m: light_setting = 'y'
                elif 'r' in m: light_setting = 's'
                else: raise ValueError
                phase_arr[i] = light_setting
            if m_mask[i] == "3": #special treatment for pedestrian xing, does OR instead of overwriting (so any green makes it green)
                if 'g' in m: light_setting = 'G'
                elif 'y' in m: light_setting = 'y'
                elif 'r' in m and phase_arr[i] == 'G': light_setting = 'G'
                elif 'r' in m and phase_arr[i] != 'G': light_setting = 'r'
                else: raise ValueError
                phase_arr[i] = light_setting
    phase_str = ''.join(phase_arr)
    phases, times = [phase_str], [math.inf]

    sim.set_phases({'J0': {"phases": phases, "times": times}})
    sim.update_lights()

def get_phases(Qcolor, Qtime, M_map, blocks_lim = None, add_end_buff = True, start_buff = '-', simulation_time = None) -> dict:
    """
    ...
    :param start_buff: Buffer character at start, i.e light setting before starting phases
    :param simulation_time: Current simulation time, if not given, new phases start immediately
    :return dict: Phases dictionary
    """

    if add_end_buff:
        Qcolor = {m: c + ['-'] for m, c in Qcolor.items()}
        Qtime  = {m: t + [math.inf] for m, t in Qtime.items()}
    ...
    validity_func = _or if allow_overlaps else xor
    if "0" in str(reduce(lambda m1, m2: '{0:b}'.format(validity_func(int(m1, 2),int(m2, 2))), list(M_map.values()))):
    ...
    if len([m for m in movements if len(Qcolor[m]) != len(Qtime[m])]) != 0:
        raise AssertionError("A movement in Qtime and Qcolor does not have matching lengths.")
    ...
    phases, times = phases_dict["phases"], phases_dict["times"]
    new_phases, new_times, skip = [], [], False
    if len(phases) == len(times):
        for i in range(len(phases) - 1):
            ...
            else: new_times.append(times[i])
        phases_dict["phases"] = new_phases
        phases_dict["times"] = new_times
    else:
        raise AssertionError("Phases and times are different lengths.\n  - {0}\n  - {1}".format(phases, times))
...
if __name__ == "__main__":
    ...
    sim = Simulation(all_vtypes=["cars", "bikes", "cbikes"])
    sim.start("bordeaux/bordeaux.sumocfg", gui='-gui' in sys.argv)

    junc_params = {'J0': {"inflow_detectors":  ["cars_n_in", "cars_e_in", "cars_s_in", "cars_w_in",
                                                "cbikes_n_in", "cbikes_e_in", "cbikes_s_in", "cbikes_w_in"],
                          "outflow_detectors": ["cars_n_out", "cars_e_out", "cars_s_out", "cars_w_out",
                                                "cbikes_n_out", "cbikes_e_out", "cbikes_s_out", "cbikes_w_out"],
                          "flow_vtypes": ["cars", "bikes", "cbikes"]}}
    sim.start_junc_tracking(junc_params)

    sim_data = sim.step_through(n_steps=800)
